...
class LocalEncoderLRMvn(nn.Module):
    def __init__(
        self, n_latents_read, input_size, hidden_size, n_latents, rank, dropout=0.0
    ):
        super().__init__()
        ...
        self.mlp = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_size, (rank + 1) * n_latents_read),
        )
    ...
    def forward(self, y):
        ...
        h = torch.nn.functional.pad(
            h, (0, self.n_latents_unread), mode="constant", value=0.0
        )
        L = torch.nn.functional.pad(
            L, (0, 0, 0, self.n_latents_unread), mode="constant", value=0.0
        )
...
class BackwardEncoderLRMvn(nn.Module):
    def __init__(
        self, n_latents_read, hidden_size, n_latents, rank_local, rank_backward
    ):
        super().__init__()
        self.n_latents_read = n_latents_read
        ...
        self.rnn = torch.nn.GRU(
            input_size=n_latents * (rank_local + 1),
            hidden_size=hidden_size,
            batch_first=True,
            bidirectional=False,
        )

        self.projection = torch.nn.Linear(hidden_size, (rank_backward + 1) * n_latents)
    ...
    def forward(self, h_y, L_y):
        ...
        h_out = torch.concat([h[:, 1:], h[:, -1:] * 0.0], dim=1)
        L_out = torch.concat([L[:, 1:], L[:, -1:] * 0.0], dim=1)
...
class BackwardEncoderDiagonal(nn.Module):
    def __init__(
        ...
        self.rnn = torch.nn.GRU(
            input_size=n_latents * 2,
            hidden_size=hidden_size,
            batch_first=True,
            bidirectional=False,
        )

        self.projection = torch.nn.Linear(hidden_size, n_latents * 2)
    ...
    def forward(self, h_y, L_y):
        ...
        h_out = torch.concat([h[:, 1:], h[:, -1:] * 0.0], dim=1)
        L_vec_out = torch.concat([L_vec[:, 1:], L_vec[:, -1:] * 0.0], dim=1)

        return h_out, L_vec_out


class BackwardEncoderDVBF(nn.Module):
    def __init__(self, n_neurons, hidden_size, n_latents):
        super().__init__()
        self.n_latents = n_latents
...
class LocalEncoderDVBF(nn.Module):
    def __init__(self, n_neurons, hidden_size, n_latents):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(n_neurons, hidden_size),
            nn.SiLU(),
            nn.Linear(hidden_size, 2 * n_latents),
        )
...
class LocalEncoderDKF(nn.Module):
    def __init__(self, n_hidden_local, n_hidden_backward, n_latents):
        super().__init__()
        self.n_latents = n_latents

        self.projection = nn.Sequential(
            nn.Linear(n_hidden_backward + n_latents, n_hidden_local),
            nn.SiLU(),
            nn.Linear(n_hidden_local, 2 * n_latents),
        )
...
class InitEncoderDKF(nn.Module):
    def __init__(self, hidden_size, n_latents):
        super().__init__()
        self.n_latents = n_latents

        self.projection = nn.Linear(hidden_size, 2 * n_latents)

...
def plot_samples_w_input(axs, samples, inputs, color_map_list):
    ...
    for input_dx in range(n_inputs):
        [axs[trial_dx_time_dx[input_dx][0][i]].axvline(trial_dx_time_dx[input_dx][1][i], linestyle='--', color='gray')
...
def plot_spikes(spikes, axs):
    ...
    for n in range(n_neurons):
        ...
        #             edgecolors='none')
        axs.scatter(neuron_spikes, 0.5 * n * np.ones_like(neuron_spikes), marker='o', color='black', s=0.1,
                    edgecolors='none')
...
def plot_single_trial_reaching_latents(z):
    ...
    with torch.no_grad():
        ...
        for n in range(n_latents):
            [ax[n].plot(z[s, :, n], color=blues(s), linewidth=0.1) for s in range(n_samples)]
...
def animate_reaching_evolution(r2_prior, r2_prd, r2_f, pos_prior, pos_prd, pos_f, reach_colors, trial_plt_dx, speeds_gt, speeds_hat, animation_path):
    xlim = 7500
    ylim = 7500
    ...
    speed_mean = [torch.cat(speeds_hat[k], dim=0).mean(dim=0) for k in range(n_start_pts)]
    ...
    with writer.saving(fig, "plots/animation.mp4", 300):
        [ax[0].plot(pos_f[n, :, 0], pos_f[n, :, 1], color=reach_colors[n]) for n in trial_plt_dx]
        ...
        for t in range(0, n_start_pts):
            ...
            for s in range(speed_gt_mean.shape[0]):
                for n in trial_plt_dx:
                    ax[1].plot(pos_prd[t][n, :s, 0], pos_prd[t][n, :s, 1], color=reach_colors[n])
            ...
            fig.savefig(f'plots/animation_t_{t}.pdf', transparent=True, bbox_inches='tight')
            if t != n_start_pts - 1:
                ax[1].clear()
                ax[2].clear()
...
def animate_reaching_evolution_mo(r2_prd, r2_f, pos_prd, pos_f, reach_colors, trial_plt_dx, speeds_gt, speeds_hat, avg_position):
# def animate_reaching_evolution_mo(r2_prd, r2_f, pos_prd, pos_f, reach_colors, trial_plt_dx, speeds_hat):
    ...
    for i in range(len(avg_position['trajectory'])):
        ax[-1].plot(avg_position['trajectory'][i][:, 0], avg_position['trajectory'][i][:, 1], color=avg_position['color'][i])
        ...
        for t in range(0, n_start_pts):
            ...
            for s in range(pos_prd[t].shape[1]):
                for n in trial_plt_dx:
                    ax[1].plot(pos_prd[t][n, :s, 0], pos_prd[t][n, :s, 1], color=reach_colors[n])
                    if t > 0 and s == 0:
                        ax[1].plot(pos_prd[t-1][n, :, 0], pos_prd[t-1][n, :, 1], color=reach_colors[n], alpha=0.25)
...
def plot_ball(y, y_hat, z_hat, w=16, label_dx=None):
    ...
    with torch.no_grad():
        ...
        for t in range(n_time_bins):
            ...
            axs[0, t].tick_params(bottom=False, left=False, labelleft=False, labelbottom=False)
            axs[1, t].tick_params(bottom=False, left=False, labelleft=False, labelbottom=False)
            # axs[0, t].axis('off')
            # axs[1, t].axis('off')
        ...
        [axbig.plot(np.arange(n_time_bins), z_hat[..., i]) for i in range(z_hat.shape[-1])]
...
def plot_ball_covariance(y, P_diag, P=None, w=16, label_dx=None):
    ...
    with torch.no_grad():
        ...
        for t in range(n_time_bins):
            ...
            axs[0, t].imshow(y_img[t], cmap='gray_r')#, vmin=0, vmax=255)
            axs[2, t].imshow(torch.diag(P_diag[t]), cmap='gray_r', vmin=min_P, vmax=max_P)#, vmin=0, vmax=255)
            if P is not None:
                axs[1, t].imshow(P[t], cmap='gray_r', vmin=min_P, vmax=max_P)#, vmin=0, vmax=255)

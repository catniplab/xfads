...
class SVAE(nn.Module):
    def __init__(self, dynamics_mod, likelihood_pdf,
                 initial_c_pdf, local_encoder, device='cpu'):
    ...
    def forward(self,
        ...
        z_s, stats = self.smooth_1_to_T(y, n_samples, p_mask_apb=p_mask_apb, p_mask_y_in=p_mask_y_in,
    ...
    def smooth_1_to_T(self,
        ...
        m_f, P_f, m_p, P_p = prob_utils.kalman_information_filter(k_y, K_y @ K_y.mT, F, Q_diag, m_0, Q_0_diag)
        m, P = prob_utils.rts_smoother(m_p, P_p, m_f, P_f, F)

        kl = self._get_kl(m, m_f, m_p, P, P_p, k_y, K_y)
        z_s = m + bmv(torch.linalg.cholesky(P), torch.randn((n_samples, n_trials, n_time_bins, m.shape[-1]), device=m.device))
    ...
    def _get_kl(self, m_s, m_f, m_p, P_s, P_p, a, A):
        ...
        kl_1 = alt_kl_step_0(m_s[:, 0], m_f[:, 0], m_0, P_s[:, 0], Q_0_diag, a[:, 0], A[:, 0])
        kl_2_T = alt_kl_step_t(m_s[:, 1:], m_f[:, 1:], m_p[:, 1:], P_s[:, 1:], P_p[:, 1:], a[:, 1:], A[:, 1:])

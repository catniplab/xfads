...
def main():
    ...
    for seed in seeds:
        ...
        y_train = torch.load("data/split_data/y_train.pt").type(torch.float32)[
            :n_trials, :n_time_bins
        ]
        z_train = torch.load("data/split_data/x_train_unscaled.pt").type(torch.float32)[
            :n_trials, :n_time_bins
        ]
        y_train = y_train.reshape((y_train.shape[0], y_train.shape[1], 256))
        y_valid = torch.load("data/split_data/y_val.pt").type(torch.float32)[
            :n_trials, :n_time_bins
        ]
        z_valid = torch.load("data/split_data/x_val_unscaled.pt").type(torch.float32)[
            :n_trials, :n_time_bins
        ]
        y_valid = y_valid.reshape((y_valid.shape[0], y_valid.shape[1], 256))
        y_test = torch.load("data/split_data/y_test.pt").type(torch.float32)[
            :n_trials, :n_time_bins
        ]
        z_test = torch.load("data/split_data/x_test_unscaled.pt").type(torch.float32)[
            :n_trials, :n_time_bins
        ]
        ...
        seq_vae = LightningPendulum.load_from_checkpoint(
            "ckpts/smoother/causal/epoch=1349_valid_mse=0.06_r2_valid_prd=0.62_r2_valid_enc=0.98_valid_loss=192.23.ckpt",
            ssm=ssm,
            cfg=cfg,
        ...
        fig.savefig(
            "plots/pendulum_predictions_causal.pdf",
            bbox_inches="tight",
            transparent=True,
        )
        fig.show()

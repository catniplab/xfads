...
def main():
    ...
    C = utils.FanInLinear(cfg.n_latents, n_neurons).requires_grad_(False)
    Q_diag = 1e-3 * torch.ones(cfg.n_latents)
    Q_0_diag = 2.0 * torch.ones(cfg.n_latents)
    ...
    y_train, z_train = y[: 2 * n_trials // 3], z[: 2 * n_trials // 3]  # noqa: F841
    y_valid, z_valid = y[2 * n_trials // 3 :], z[2 * n_trials // 3 :]  # noqa: F841
    ...
    """likelihood pdf"""
    likelihood_pdf = GaussianLikelihood(C, n_neurons, R_diag)

...
def main():
    ...
    test_dataloader = torch.utils.data.DataLoader(
        y_test_dataset, batch_size=y_test_obs.shape[0], shuffle=False
    )
    ...
    z_ic = m_0 + Q_0.sqrt() * torch.randn(
        [cfg.n_samples] + [batch_sz_test[0]] + [cfg.n_latents]
    )
    ...
    rates_test_p = [
        cfg.bin_sz
        * torch.exp(seq_vae.ssm.likelihood_pdf.readout_fn(z_p_test[k])).mean(dim=0)
        for k in range(predict_start_max)
    ]
    rates_test_ic_p = cfg.bin_sz * torch.exp(
        seq_vae.ssm.likelihood_pdf.readout_fn(z_ic_p)
    ).mean(dim=0)
    ...
    with torch.no_grad():
        ...
        r2_test_ic_p = clf.score(
            rates_test_ic_p.reshape(-1, n_neurons_obs), vel_test.reshape(-1, 2)
        )
        r2_test_p = [
            clf.score(
                rates_test_p[k].reshape(-1, n_neurons_obs), vel_test.reshape(-1, 2)
            )
            for k in range(predict_start_max)
        ]
        ...
        vel_hat_test_ic_p = clf.predict(
            rates_test_ic_p.reshape(-1, n_neurons_obs)
        ).reshape(list(batch_sz_test) + [2])
        vel_hat_test_p = [
            clf.predict(rates_test_p[k].reshape(-1, n_neurons_obs)).reshape(
                list(batch_sz_test) + [2]
            )
            for k in range(predict_start_max)
        ]
        ...
        pos_test_hat_p = [
            vel_to_pos(vel_hat_test_p[k]) for k in range(predict_start_max)
        ]
    ...
    for trial_dx in torch.arange(0, 108, 3):
        ...
        speed_hat = [
            torch.tensor(
                vel_hat_test_p[k][cond_trials][..., 0] ** 2
                + vel_hat_test_p[k][cond_trials][..., 1] ** 2
            )
            .sqrt()
            .mean(dim=[0])
            for k in range(predict_start_max)
        ...
        speed_gt = (
            torch.tensor(
                vel_test[cond_trials][..., 0] ** 2 + vel_test[cond_trials][..., 1] ** 2
            )
            .sqrt()
            .mean(dim=[0])
        )
        speed_padded = [
            torch.cat(
                [torch.zeros(5), torch.nan_to_num(speed_hat[k][:-5], 0.0)], dim=-1
            )
            for k in range(predict_start_max)
        ]
        speed_gt_padded = torch.cat(
            [torch.zeros(5), torch.nan_to_num(speed_gt[:-5], 0.0)], dim=-1
        )
        [
            speeds[k].append(speed_padded[k].unsqueeze(0))
            for k in range(predict_start_max)
        ]
    ...
    plot_utils.animate_reaching_evolution(
        r2_test_ic_p,
        r2_test_p,
        r2_test_f,
        pos_test_hat_ic_p,

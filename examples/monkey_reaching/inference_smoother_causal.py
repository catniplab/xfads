...
def main():
    ...
    for seed in [1234, 1235, 1236]:
        ...
        """create ssm"""
        ssm = create_xfads_poisson_log_link(cfg, n_neurons_obs, train_dataloader, model_type='c')

        """lightning"""
        seq_vae = LightningMonkeyReaching(ssm, cfg, n_time_bins_enc, bin_prd_start)

        csv_logger = CSVLogger('logs/smoother/causal/',
        ...
        ckpt_callback = ModelCheckpoint(save_top_k=3, monitor='r2_valid_enc', mode='max',
                                        dirpath=f'ckpts/smoother/causal_mask_{cfg.p_mask_a}/', save_last=True,

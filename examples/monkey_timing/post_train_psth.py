...
from xfads.smoothers.lightning_trainers import LightningDMFCRSG
...
def main():
    ...
    for seed in seeds:
        ...
        train_data = torch.load(
            data_path.format(split="train", bin_sz_ms=cfg.bin_sz_ms)
        )
        ...
        y_train_dataset = torch.utils.data.TensorDataset(
            y_train_obs, ts_train, task_id_train
        )
        y_val_dataset = torch.utils.data.TensorDataset(
            y_valid_obs, ts_valid, task_id_valid
        )
        y_test_dataset = torch.utils.data.TensorDataset(
            y_test_obs, ts_test, task_id_test
        )
        ...
        """lightning"""
        seq_vae = LightningDMFCRSG.load_from_checkpoint(
            "ckpts/smoother/acausal/model_path.ckpt",
            ssm=ssm,
            cfg=cfg,
        ...
        _, z_s, _ = seq_vae.ssm(y_test_obs[:, :n_time_bins_enc], cfg.n_samples)
        z_p = seq_vae.ssm.predict_forward(
            z_s[:, :, n_bins_bhv], n_time_bins_enc - n_bins_bhv
        )

        # smoothed and predicted log rates
        r_s = seq_vae.ssm.likelihood_pdf.readout_fn(z_s)
        r_p = seq_vae.ssm.likelihood_pdf.readout_fn(z_p)
        ...
        with torch.no_grad():
            ...
            plot_utils.plot_spikes(y_test_obs[i], axs[0])
            plot_utils.plot_spikes(y_hat[0, i], axs[1])

            fig.suptitle(i)
            fig.savefig(
                "plots/dmfc_spikes_predictions_acausal.svg",
                bbox_inches="tight",
                transparent=True,
            )
            fig.show()
    ...
    for task_id in [4, 7, 8, 10, 11]:
        ...
        for k in range(50):
            ...
            with torch.no_grad():
                # ground truth psth
                y_psth = torch.tensor(
                    gaussian_filter1d(y_subset.mean(dim=0), sigma=10, axis=0)
                )
                # psth sampled from generative model
                y_hat_psth = torch.tensor(
                    gaussian_filter1d(y_hat_subset.mean(dim=[1]), sigma=10, axis=1)
                )

                axs[k].plot(y_psth[:, neuron_plot_id], color="blue")
                [
                    axs[k].plot(
                        y_hat_psth[j, :, neuron_plot_id],
                        color=colors(j),
                        linewidth=0.5,
                        alpha=0.5,
                    )
        ...
        fig.savefig(
            f"plots/dmfc_psth_predictions_acausal_id_{task_id}.pdf",
            bbox_inches="tight",
            transparent=True,
        )
        plt.show()

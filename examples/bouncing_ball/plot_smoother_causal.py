...
def main():
    ...
    for seed in seeds:
        ...
        y_train = torch.load("data/neural_ssm/y_train.pt").type(torch.float32)[
            :n_trials, :n_time_bins
        ]
        z_train = torch.load("data/neural_ssm/x_train.pt").type(torch.float32)[
            :n_trials, :n_time_bins
        ]
        y_train = y_train.reshape((y_train.shape[0], y_train.shape[1], 256))
        y_valid = torch.load("data/neural_ssm/y_valid.pt").type(torch.float32)[
            :n_trials, :n_time_bins
        ]
        z_valid = torch.load("data/neural_ssm/x_valid.pt").type(torch.float32)[
            :n_trials, :n_time_bins
        ]
        y_valid = y_valid.reshape((y_valid.shape[0], y_valid.shape[1], 256))
        y_test = torch.load("data/neural_ssm/y_test.pt").type(torch.float32)[
            :n_trials, :n_time_bins
        ]
        z_test = torch.load("data/neural_ssm/x_test.pt").type(torch.float32)[
            :n_trials, :n_time_bins
        ]
        ...
        valid_dataloader = torch.utils.data.DataLoader(
            y_valid_dataset, batch_size=cfg.batch_sz, shuffle=True
        )
        test_dataloader = torch.utils.data.DataLoader(
            y_test_dataset, batch_size=cfg.batch_sz, shuffle=True
        )

        """likelihood pdf"""
        R_diag = torch.ones(n_neurons)
        H = utils.ReadoutLatentMask(cfg.n_latents, cfg.n_latents_read)
        readout_fn = nn.Sequential(
            H,
            nn.Linear(cfg.n_latents_read, 128),
            nn.SiLU(),
            nn.Linear(128, n_neurons),
            nn.Sigmoid(),
        )
        likelihood_pdf = GaussianLikelihood(readout_fn, n_neurons, R_diag)
        ...
        """lightning"""
        ckpt_path = "ckpts/smoother/causal/epoch=1999_valid_loss=-4895.67_r2_valid_prd=0.39_r2_valid_enc=0.86-v1.ckpt"
        seq_vae = LightningPendulum.load_from_checkpoint(
            ckpt_path, ssm=ssm, cfg=cfg, n_time_bins_enc=n_time_bins_enc, strict=False
        )
        _, z_s, stats_s = seq_vae.ssm(
            y_test[:, :n_time_bins_enc], cfg.n_samples, get_P_s=True
        )
        z_p = seq_vae.ssm.predict_forward(z_s[:, :, -1], n_time_bins - n_time_bins_enc)

        y_s = seq_vae.ssm.likelihood_pdf.readout_fn(z_s)
        y_p = seq_vae.ssm.likelihood_pdf.readout_fn(z_p)
        ...
        end_dx = 75
        start_dx = 40
        fig = plot_utils.plot_ball(
            y_test[i, start_dx:end_dx],
            y_hat[0, i, start_dx:end_dx],
            z_hat[0, i, start_dx:end_dx],
            label_dx=50 - start_dx,
        )
        fig.savefig(
            "plots/bouncing_ball_predictions_causal.pdf",
            bbox_inches="tight",
            transparent=True,
        )
        fig.show()
        ...
        end_dx = 50
        start_dx = 10
        fig = plot_utils.plot_ball_covariance(
            y_test[i, start_dx:end_dx],

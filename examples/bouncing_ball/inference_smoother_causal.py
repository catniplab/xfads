...
def main():
    ...
    for seed in seeds:
        ...
        y_train = torch.load('data/neural_ssm/y_train.pt').type(torch.float32)[:n_trials, :n_time_bins]
        z_train = torch.load('data/neural_ssm/x_train.pt').type(torch.float32)[:n_trials, :n_time_bins]
        y_train = y_train.reshape((y_train.shape[0], y_train.shape[1], 256))
        y_valid = torch.load('data/neural_ssm/y_valid.pt').type(torch.float32)[:n_trials, :n_time_bins]
        z_valid = torch.load('data/neural_ssm/x_valid.pt').type(torch.float32)[:n_trials, :n_time_bins]
        y_valid = y_valid.reshape((y_valid.shape[0], y_valid.shape[1], 256))
        y_test = torch.load('data/neural_ssm/y_test.pt').type(torch.float32)[:n_trials, :n_time_bins]
        z_test = torch.load('data/neural_ssm/x_test.pt').type(torch.float32)[:n_trials, :n_time_bins]
        ...
        test_dataloader = torch.utils.data.DataLoader(y_test_dataset, batch_size=cfg.batch_sz, shuffle=False)
        ...
        """dynamics module"""
        # dynamics_fn = utils.MGU(cfg.n_hidden_dynamics, cfg.n_latents, device=cfg.device)
        ...
        torch.save(ckpt_callback.best_model_path, 'ckpts/smoother/causal/best_model_path.pt')

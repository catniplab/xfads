...
def main():
    ...
    for seed in seeds:
        ...
        seq_vae = LightningPendulum.load_from_checkpoint(ckpt_path, ssm=ssm,
                                                         cfg=cfg, n_time_bins_enc=n_time_bins_enc, strict=False)
        ...
        y_s = seq_vae.ssm.to('cpu').likelihood_pdf.readout(z_s)
        y_p = seq_vae.ssm.to('cpu').likelihood_pdf.readout(z_p)
        ...
        fig.savefig('plots/bouncing_ball_predictions_acausal.pdf', bbox_inches='tight', transparent=True)
        fig.show()
